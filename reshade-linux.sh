...
function downloadD3dcompiler_47() {
    ...
    printf "Downloading d3dcompiler_47.dll for $1 bits."
    createTempDir
    ...
    curl -sLO "https://download-installer.cdn.mozilla.net/pub/firefox/releases/62.0.3/win$1/ach/Firefox%20Setup%2062.0.3.exe" \
        || printf "Could not download Firefox setup file (which contains d3dcompiler_47.dll)"
...
function linkShaderFiles() {
    ...
    for file in *; do
        ...
        printf "Linking $INFILE to $OUTDIR"
        ln -s "$INFILE" "$OUTDIR"
    done
}
...
if [[ -n $SHADER_REPOS ]]; then
    printf "Checking for ReShade Shader updates."
    ...
    for URI in $(echo "$SHADER_REPOS" | tr ';' '\n'); do
        localRepoName=$(printf "$URI" | cut -d'|' -f2)
        branchName=$(printf "$URI" | cut -d'|' -f3)
        URI=$(printf "$URI" | cut -d'|' -f1)
        if [[ -d "$MAIN_PATH/ReShade_shaders/$localRepoName" ]]; then
            if [[ $UPDATE_RESHADE -eq 1 ]]; then
                ...
                printf "Updating ReShade shader repository $URI."
    ...
    if [[ $MERGE_SHADERS == 1 ]] && [[ -d "$MAIN_PATH/External_shaders" ]]; then
        printf "Checking for External Shader updates."
...
if [[ $RESHADE_VERSION != latest ]]; then
    ...
    if [[ ! -f reshade/$RESHADE_VERSION/ReShade64.dll ]] || [[ ! -f reshade/$RESHADE_VERSION/ReShade32.dll ]]; then
        printf "Downloading version $RESHADE_VERSION of ReShade.\n$SEPERATOR\n"
    ...
    printf "Using version $RESHADE_VERSION of ReShade.\n"
else
    printf "Using the latest version of ReShade ($LVERS).\n"
fi
# Z0016
...
# Z0030
printf "Do you want to (i)nstall or (u)ninstall ReShade for a DirectX or OpenGL game?"
if [[ $(checkStdin "(i/u): " "^(i|u)$") == "u" ]]; then
    getGamePath
    printf "Unlinking ReShade files."
    LINKS="$(printf '%s.dll' "$COMMON_OVERRIDES" | awk '{gsub(/ /, ".dll "); print $0 ".dll"}') ReShade.ini ReShade32.json ReShade64.json d3dcompiler_47.dll Shaders Textures ReShade_shaders ${LINK_PRESET}"
    for link in $LINKS; do
        if [[ -L $gamePath/$link ]]; then
            printf "Unlinking \"$gamePath/$link\"."
            unlink "$gamePath/$link"
        fi
    done
    if [[ $DELETE_RESHADE_FILES == 1 ]]; then
        printf "Deleting ReShade.log and ReShadePreset.ini"
        rm -f "$gamePath/ReShade.log" "$gamePath/ReShadePreset.ini"
    fi
    printf "Finished uninstalling ReShade for '$gamePath'."
    printf "\e[40m\e[32mMake sure to remove or change the \e[34mWINEDLLOVERRIDES\e[32m environment variable.\e[0m"
    exit 0
fi
# Z0030
...
if [[ $wantedDll == "manual" ]]; then
    ...
    while true; do
        ...
        printf "You have entered '$wantedDll', is this correct?"
        read -rp '(y/n): ' ynCheck
...
if [[ $exeArch == 32 ]]; then
    printf "Linking ReShade32.dll to $wantedDll.dll."
    ln -is "$(realpath "$RESHADE_PATH/$RESHADE_VERSION"/ReShade32.dll)" "$gamePath/$wantedDll.dll"
else
    printf "Linking ReShade64.dll to $wantedDll.dll."
...
if [[ -f $MAIN_PATH/$LINK_PRESET ]]; then
    printf "Linking $LINK_PRESET to game directory."
