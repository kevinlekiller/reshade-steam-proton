...
if [[ $(checkStdin "(i/u): " "^(i|u)$") == "u" ]]; then
    ...
    LINKS="$(printf '%s' "$COMMON_OVERRIDES" | awk '{gsub(/ /, ".dll "); print $0 ".dll"}') ReShade.ini ReShade32.json ReShade64.json d3dcompiler_47.dll Shaders Textures ReShade_shaders ${LINK_PRESET}"
